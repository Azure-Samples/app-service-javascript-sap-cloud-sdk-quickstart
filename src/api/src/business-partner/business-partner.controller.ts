...
export class BusinessPartnerController {
  ...
  async getBusinessPartners (@Headers() headers): Promise<BusinessPartner[]> {
    ...
    return await this.businessPartnerService
      ...
      .catch(error => {
        throw this.createHttpException(`Failed to get business partners`, error);
      })
  }

  @Get('/:id')
  async getBusinessPartnerById (@Param('id') id: string, @Headers() headers): Promise<BusinessPartner> {
    // retrieve X-MS-TOKEN-AAD-ACCESS-TOKEN token from request header
    const token = headers['x-ms-token-aad-access-token']
    return await this.businessPartnerService
      .getBusinessPartnerById(id, token)
      .catch(error => {
        throw this.createHttpException(`Failed to get business partner by id ${id}`, error);
      })
  }
  ...
  async createAddress (
    ...
    return await this.businessPartnerService
      ...
      .catch(error => {
        throw this.createHttpException(`Failed to create address for business partner ${id}`, error);
      })
  }
  ...
  async updateBusinessPartnerAddress (
    ...
    return await this.businessPartnerService
      ...
      .catch(error => {
        throw this.createHttpException(`Failed to update Business Partner ${businessPartnerId} address ${addressId}`, error);
      })
  }
  ...
  async deleteBusinessPartnerAddress (
    ...
    return await this.businessPartnerService
      ...
      .catch(error => {
        throw this.createHttpException(`Failed to delete address ${addressId} for business partner ${businessPartnerId}`, error);
      })
  }
  ...
  private getFaultString (data: any): string {
    ...
    else if(data.fault !== undefined && data.fault.faultstring !== undefined){
